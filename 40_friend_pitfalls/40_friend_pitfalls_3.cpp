/*
	...
	to allow a security flaw, however, you'll learn how to use friend in C++.
*/
#include <iostream>
using namespace std;
...
class BaseClass {
	...
	public:
		...
		*/
		...
		friend void accessForEveryone(const BaseClass &bc);

		/*	compiler error: 'DerivedClass' does not name a type	*/
		// friend void accessForEveryone(const DerivedClass &dc);
};
...
class DerivedClass : public BaseClass {
	...
	private:
		string privateDerived;

	protected:
		string protectedDerived;
...
class ForeignClass {
	public:
		void access0(const DerivedClass &dc) {
			...
			cout << "private super class member: " << dc.private_member << endl
			<< "protected super class member: " << dc.protected_member << endl;
		...
		void access1(const BaseClass &bc) {
			...
			cout << "private super class member: " << bc.private_member << endl
			<< "protected super class member: " << bc.protected_member << endl;

			//	without const this is also able to call
			/*bc.unable_to_display();*/

			/* cout << bc.privateDerived << endl;	*/
		}
		...
		void access2(const BaseClass &bc) {
			...
			cout	<< "private member: " << dc.private_member << endl
					<< "the protected member from dc: " << dc.print_protected_member() << endl;
...
void accessForEveryone(const BaseClass &bc) {
...
void accessForEveryone(const DerivedClass &dc) {
...
int main() {
	...
	// accessForEveryone(fc);
	accessForEveryone(bc);
	accessForEveryone(dc);
